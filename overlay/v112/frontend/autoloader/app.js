(function () {
  ...
  function poopsUrl(payload) {
    return 'slopkit/slopkit/poops.html?go=1&auto=1&production=1&trigger=netcontrol&attempts=8&only=ps0_preflight,ps1_prepare,ps3_stage0,ps4_validate,ps5_stage1,ps6_stage2,ps8_stage3,ps9_stage4,ps10_stage5&log=debug&payload=1&autoload=' + encodeURIComponent(payload) + '&v=goldengames112';
  }
  function p2jbUrl(payload) {
    return 'slopkit/slopkit/p2jb.html?go=1&auto=1&production=1&log=debug&payload=1&autoload=' + encodeURIComponent(payload) + '&v=goldengames112';
  }
  ...
  function onAutoloadResult(data) {
    ...
    if (data.ok) {
      ...
      try {
        localStorage.setItem('goldengames:session-ready', String(Date.now()));
        localStorage.setItem('goldengames:active-payload', selectedPayload);
      } catch (e) { }
  ...
  function launchSelected(payload, label, forceJailbreak) {
    ...
    try {
      ...
      if (forceJailbreak) localStorage.removeItem('goldengames:session-ready');
    } catch (e) { }
  ...
  function requestPayloadLaunch(payload, label, risk, forceJailbreak) {
    ...
    var active = '';
    try { active = localStorage.getItem('goldengames:active-payload') || ''; } catch (e) { }
  ...
  function start() {
    uiLog('Goldengames PS5 Autoloader v1.1.2', 'success');
    ...
    if (riskContinueEl) riskContinueEl.addEventListener('click', function () {
      ...
      if (riskDialogEl) riskDialogEl.hidden = true;
      try {
        localStorage.removeItem('goldengames:session-ready');
        localStorage.removeItem('goldengames:active-payload');
      } catch (e) { }
      launchSelected(pending.payload, pending.label, pending.force);
    });

    var knownSession = false;
    try { knownSession = !!localStorage.getItem('goldengames:session-ready'); } catch (e) { }
    if (knownSession) {
