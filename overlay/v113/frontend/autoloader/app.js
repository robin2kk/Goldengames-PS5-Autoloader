(function () {
  ...
  var persistentSenderReady = false;
  var persistentSendTimer = 0;
  var SESSION_KEY = 'goldengames:v113-session-ready';
  var ACTIVE_PAYLOAD_KEY = 'goldengames:v113-active-payload';
  ...
  function poopsUrl(payload) {
    return 'slopkit/slopkit/poops.html?go=1&auto=1&production=1&trigger=netcontrol&attempts=8&only=ps0_preflight,ps1_prepare,ps3_stage0,ps4_validate,ps5_stage1,ps6_stage2,ps8_stage3,ps9_stage4,ps10_stage5&log=debug&payload=1&autoload=' + encodeURIComponent(payload) + '&v=goldengames113';
  }
  function p2jbUrl(payload) {
    return 'slopkit/slopkit/p2jb.html?go=1&auto=1&production=1&log=debug&payload=1&autoload=' + encodeURIComponent(payload) + '&v=goldengames113';
  }
  ...
  function onAutoloadResult(data) {
    ...
    collapseP2jbStats();
    if (data.ok) {
      if (exploitMode === 'umtx2') persistentSenderReady = true;
      ...
      if (exploitMode === 'umtx2' && !persistentSenderReady) {
  ...
  function launchSelected(payload, label, forceJailbreak) {
    if (!forceJailbreak && persistentSenderReady && exploitMode === 'umtx2') {
      sendViaPersistentSender(payload, label);
      return;
    }
    if (forceJailbreak && persistentSenderReady) {
  ...
  function sendViaPersistentSender(payload, label) {
    selectedPayload = payload;
    selectedLabel = label;
    finished = false;
    chainStarted = true;
    mirroredLines = 0;
    if (goldenStateEl) goldenStateEl.textContent = 'SENDING · ' + label.toUpperCase();
    updateProgress(18, 'Sending ' + label + ' directly to elfldr 9021...');
    revealExploit();
    ...
    persistentSendTimer = setTimeout(function () {
      ...
      persistentSenderReady = false;
      chainStarted = false;
      updateProgress(0, 'Sender session lost. Reopen Goldengames to rebuild it.');
      uiLog('[ERROR] Live elfldr sender did not respond.', 'error');
    }, 12000);
  ...
  function start() {
    uiLog('Goldengames PS5 Autoloader v1.1.3', 'success');
